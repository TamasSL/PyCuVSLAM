...
# global cache for coordinate grids to avoid recreation every frame
_coords_grid_cache = {}
_meshgrid_cache = {}
...
def coords_grid(ht, wd, **kwargs):
    """cached coordinate grid generation"""
    device = kwargs.get('device', 'cpu')
    cache_key = (ht, wd, str(device))

    if cache_key not in _coords_grid_cache:
        y, x = torch.meshgrid(
            torch.arange(ht, dtype=torch.float, **kwargs),
            torch.arange(wd, dtype=torch.float, **kwargs),
            indexing="ij",
        )
        _coords_grid_cache[cache_key] = torch.stack([x, y], dim=-1)

    return _coords_grid_cache[cache_key]


def _get_meshgrid(ht, wd, device):
    """cached meshgrid for projection operations"""
    cache_key = (ht, wd, str(device))
    ...
    if cache_key not in _meshgrid_cache:
        ...
        _meshgrid_cache[cache_key] = (y, x)

    return _meshgrid_cache[cache_key]
...
def iproj(disps, intrinsics, jacobian=False):
    ...
    # use cached meshgrid
    y, x = _get_meshgrid(ht, wd, disps.device)
...
def induced_flow(poses, disps, intrinsics, ii, jj):
    ...
    ht, wd = disps.shape[2:]
    # use cached coordinate grid
    coords0 = coords_grid(ht, wd, device=disps.device)
