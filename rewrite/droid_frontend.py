...
class DroidFrontend:
    def __init__(self, net, video, args):
        ...
        # cached pose inverse for motion damping (avoid recomputing)
        self._cached_pose_inv = None
        self._cached_pose_idx = -1
    ...
    def _init_next_state(self):
        ...
        # damped linear velocity model with cached pose inverse
        if self.motion_damping >= 0:
            poses = SE3(self.video.poses)
            # cache previous pose inverse to avoid recomputation
            prev_idx = self.t1 - 2
            if self._cached_pose_idx != prev_idx or self._cached_pose_inv is None:
                self._cached_pose_inv = poses[prev_idx].inv()
                self._cached_pose_idx = prev_idx
            vel = (poses[self.t1 - 1] * self._cached_pose_inv).log()
