...
class FactorGraph:
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        with autocast(enabled=False):
            ...
            # apply height-based weight reduction if enabled
            # this reduces BA influence of points outside camera-relative height range
            if self.video.HEIGHT_FILTER is not None:
                height_mask = self.video.compute_height_mask(ii)
                if height_mask is not None:
                    # reshape mask to match weight: (N, 2, H, W)
                    height_weight = height_mask.float().unsqueeze(1).expand_as(weight)
                    weight = weight * height_weight
