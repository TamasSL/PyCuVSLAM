...
class DroidBackend:
    # constant max_factors to prevent O(N) growth in backend optimization
    MAX_BACKEND_FACTORS = 2048
    ...
    def __call__(self, steps=12, normalize=True):
        ...
        # use constant max_factors instead of 16*t to prevent O(N) scaling
        graph = FactorGraph(self.video, self.update_op, corr_impl="alt", max_factors=self.MAX_BACKEND_FACTORS, upsample=self.upsample)
...
class DroidAsyncBackend:
    # constant max_factors to prevent unbounded growth
    MAX_ASYNC_FACTORS = 2048
    ...
    def __init__(self, net, video, args, max_age = 7):
        ...
        self.graph = FactorGraph(
            ...
            self.update_op,
            corr_impl="alt",
            max_factors=self.MAX_ASYNC_FACTORS,  # bounded instead of -1
